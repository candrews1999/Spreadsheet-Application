...
public final class WorksheetReader {
  ...
  public interface WorksheetBuilder<T> {
    ...
    /**
     * Changes the size of a column in the Worksheet.
     *
     * @param col  the column to be resized (1-indexed)
     * @param size the size of the column
     * @return this {@link WorksheetBuilder}
     */
    WorksheetBuilder<T> changeColSize(int col, int size);

    /**
     * Changes the size of a row in the Worksheet.
     *
     * @param row  the row to be resized (1-indexed)
     * @param size the size of the row
     * @return this {@link WorksheetBuilder}
     */
    WorksheetBuilder<T> changeRowSize(int row, int size);
   ...
   * <p>Row and column sizes are represented with "ROW" or "COL", followed by the corresponding row
   * or column number, followed by the new size of the row or column.</p>
   *
  ...
  public static <T> T read(WorksheetBuilder<T> builder, Readable readable) {
    ...
    while (scan.hasNext()) {
      ...
      switch (next) {
        case "COL":
          scan.skip("\\s*");
          int colRef = checkInt(scan.next());
          scan.skip("\\s*");
          int colSize = checkInt(scan.next());
          builder = builder.changeColSize(colRef, colSize);
          break;
        case "ROW":
          scan.skip("\\s*");
          int rowRef = checkInt(scan.next());
          scan.skip("\\s*");
          int rowSize = checkInt(scan.next());
          builder = builder.changeRowSize(rowRef, rowSize);
          break;
        default:
          Matcher m = cellRef.matcher(next);
          if (m.matches()) {
  ...
  private static int checkInt(String next) {
    try {
